...
This can be seen in figures 5(a) through to 5(c) in Figure~\ref{graph:reno-down}. The discrepancies in 5(b) can be attributed to the unstable nature of wireless environments.
...
sense conceptually; on the uplink case, the 802.11 MAC provides fairness
\textbf{per 802.11 station} using carrier sense, and on the downlink case
fairness is provided by carrier sense \textbf{and} the APs. Since Multipath TCP
acts as one station on each network, one would expect each subflow to get half
...
this will be looked at in \S\ref{sec:results-performance}.
