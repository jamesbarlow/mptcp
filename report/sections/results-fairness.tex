...
Coupled to be fair with regard to the other flows; that is, the
...
After seeing similar results for same-channel, non-overlapping and cross-band  
...
send queue will simply grow, but not drop packets. TCP counts the
...
This is not common, as a TCP flow will start seeing loss when it overflows a
...
For regular TCP flows, since the NIC always
...
send queue with packets that the NIC is unable to send. This is a problem because any
packets sent by other flows on the same host will now be queued behind these
packets, increasing delay significantly for these packets, which is a problem
for flows where
...
drastically reduced and is lower than that of the parallel clients.
