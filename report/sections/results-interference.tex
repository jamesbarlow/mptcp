...
would like to explore whether the nature of wireless permits transmitting
...
utilization measurement show completely fair sharing of aggregate
...
both links. The reason why this behaviour is expected is that due to the networks being on opposite ends of the 2.4 GHz spectrum, they should not interfere with each other, and
...
other band is in use. This plot also shows a case where the 2.4 GHz
parallel is in fact performing better than its single counterpart. As mentioned
above, we put this down to the background traffic on other networkss changing
